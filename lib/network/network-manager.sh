...
cmd_show() {
  ...
  echo ""; echo "Datacenter Network Details:" && virsh net-info "$NETWORK_NAME" 2>/dev/null || echo "$NETWORK_NAME not found"

  echo ""; echo "Active DHCP Leases (via virsh):"
  local dhcp_leases
  ...
  echo ""; echo "Active port forwarding rules (iptables):"
  local nat_rules
  ...
  if [ -f "$port_file" ]; then
    ...
    echo ""; echo "Legend: ✓ = accessible, ✗ = not accessible"
  else
...
cmd_bridge() {
  ...
  echo ""; echo "Routes:"; ip route show | sed 's/^/  /'
  echo ""; echo "dnsmasq:"; ps aux | grep "dnsmasq.*${BRIDGE_NAME}" | grep -v grep || echo "No dnsmasq process for bridge"
}
...
cmd_ip_forwarding() {
  ...
  case "$action" in
    on|enable)
      echo 1 > /proc/sys/net/ipv4/ip_forward || { print_error "Failed to enable ip_forward"; exit 1; }
      print_success "Enabled net.ipv4.ip_forward (not persisted)"
      ;;
    off|disable)
      echo 0 > /proc/sys/net/ipv4/ip_forward || { print_error "Failed to disable ip_forward"; exit 1; }
