...
if [ "$FORCE_MODE" = true ]; then
	...
	[[ ! "$VM_DISK_SIZE" =~ ^[0-9]+[GMT]$ ]] && { print_error "Disk size format: number + G/M/T (e.g., 20G, 512M, 1T)"; exit 1; }
	size_num=$(echo "$VM_DISK_SIZE" | sed 's/[GMT]$//')
	size_unit=$(echo "$VM_DISK_SIZE" | sed 's/^[0-9]*//')
	case "$size_unit" in
	"M") [ "$size_num" -lt 100 ] && { print_error "Minimum disk size is 100M"; exit 1; } ;;
	"G") [ "$size_num" -lt 1 ] || [ "$size_num" -gt 1000 ] && { print_error "Disk size must be between 1G and 1000G"; exit 1; } ;;
	"T") [ "$size_num" -gt 10 ] && { print_error "Maximum disk size is 10T"; exit 1; } ;;
	esac
else
...
echo "   Setup ports: dcvm network ports setup"
