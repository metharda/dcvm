...
fix_vm_locks() {
  ...
  if command_exists qemu-img; then
    ...
    qemu-img info "$VM_DISK_PATH" >/dev/null 2>&1 && print_verbose "qemu-img can access the disk" || {
    ...
    echo "$qemu_check_output" | grep -qi "lock\|busy\|use" && {
...
" 2>/dev/null || {
  ...
  if [ -f "$VM_DISK_PATH" ]; then
    ...
    chmod 660 "$VM_DISK_PATH" 2>/dev/null && print_success "Fixed permissions: $target_owner with 660" || {
  ...
  check_vm_exists "$vm_name" && print_verbose "VM is properly defined in libvirt" || {
  ...
  virsh dominfo "$vm_name" >/dev/null 2>&1 && print_verbose "VM domain info accessible" || {
  ...
  echo ""
  [ $step_errors -eq 0 ] && print_success "=== Lock Fix Complete for $vm_name (No Errors) ===" || {
...
main() {
  ...
  if [ "$VM_NAME" = "--all" ]; then
    ...
    [ $failed_vms -gt 0 ] && print_warning "Some VMs had issues during lock fix" && exit 1 || {
      print_success "Lock fix completed successfully for all VMs"
      exit 0
    }
  else
    fix_vm_locks "$VM_NAME" && exit 0 || {
