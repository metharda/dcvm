...
show_port_status() {
    ...
    # Show active port forwarding rules
...
start_vms() {
    ...
    if [ "$target" = "all" ] || [ -z "$target" ]; then
        ...
        # Get list of stopped VMs that use datacenter-net
...
stop_vms() {
    ...
    if [ "$target" = "all" ] || [ -z "$target" ]; then
        ...
        # Get list of running VMs that use datacenter-net
...
restart_vms() {
    ...
    if [ "$target" = "all" ] || [ -z "$target" ]; then
        ...
        # Get list of all VMs that use datacenter-net
...
show_enhanced_status() {
    ...
    # Show VM IP addresses dynamically
...
case $1 in
    ...
    "list")
        ...
        # Show which VMs use datacenter-net
