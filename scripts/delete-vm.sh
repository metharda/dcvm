...
# Function to delete a single VM with comprehensive cleanup
delete_single_vm() {
    local VM_NAME="$1"
    ...
    # Get MAC address first (needed for DHCP cleanup)
    ...
    # Get VM disk path for complete storage cleanup
    ...
    # Try to get port mappings from file
    if [ -f /srv/datacenter/port-mappings.txt ]; then
        ...
        if [ -n "$mapping_line" ]; then
            ...
            # Use IP from file if we couldn't detect it
            if [ -z "$VM_IP" ]; then
    ...
    if [ -n "$VM_IP" ]; then
        ...
        # Remove specific port rules if we know them
        if [ -n "$SSH_PORT" ] && [ -n "$HTTP_PORT" ]; then
            ...
            # Remove any rules pointing to this IP (fallback method)
            print_status "Searching and removing all rules with IP $VM_IP..."
            # Store rules to remove (avoid modifying while reading)
        ...
        # Try to remove rules that might contain the VM name
    ...
    # Clear DHCP lease records comprehensively
    print_status "Clearing DHCP lease records..."
    
    # Method 1: Clear by MAC address
    if [ -n "$MAC_ADDRESS" ]; then
    ...
    # Clean additional DHCP lease locations
    ...
    # Remove from port mappings file
    if [ -f /srv/datacenter/port-mappings.txt ]; then
    ...
    # Remove VM definition with all storage
    ...
    if virsh undefine "$VM_NAME" --remove-all-storage 2>/dev/null; then
        ...
        # Fallback: try without --remove-all-storage and manual cleanup
    ...
    if [ -f ~/.ssh/config ]; then
        # More comprehensive SSH config cleanup
    ...
    # Remove any systemd services related to this VM
    ...
    # Refresh DHCP without destroying network
    print_status "Refreshing DHCP leases..."
    # Send SIGHUP to dnsmasq to reload without destroying network
...
# Function to delete all datacenter VMs with extreme safety checks
delete_all_vms() {
    print_warning "======================================="
    ...
    # Second confirmation with VM count
    ...
    # Additional cleanup after all VMs are deleted
    print_status "Performing final cleanup..."
    
    # Clear all DHCP leases WITHOUT destroying network
    ...
    # Remove all datacenter port forwarding rules
