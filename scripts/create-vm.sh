...
show_usage() {
	...
	echo "  -f, --force                    Force mode with interactive prompts for unspecified options"
	...
	echo ""
	echo "Force Mode with Partial Config (prompts for unspecified):"
	echo "  $0 vm1 -f -u admin -p mypass                      # Prompts for memory, CPU, disk, OS, root"
	echo "  $0 vm2 -f -p mypass -m 4096 -c 4                  # Prompts for username, disk, OS, root"
	echo "  $0 vm3 -f -p mypass -o 3                          # Prompts for username, memory, CPU, disk, root"
	echo ""
